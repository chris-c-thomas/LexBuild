...
LexBuild is a platform for converting legislative XML into structured Markdown optimized for AI ingestion. It supports two source formats -- U.S. Code (USLM 1.0 schema) and eCFR (GPO/SGML-derived XML) -- with an architecture designed for additional sources.
...
- **Frontmatter Generator** -- Produces YAML frontmatter (FORMAT_VERSION 1.1.0) with multi-source support. Every output file includes a `source` discriminator (`"usc"` or `"ecfr"`) and a `legal_status` field (`"official_legal_evidence"`, `"official_prima_facie"`, or `"authoritative_unofficial"`). Source-specific optional fields are included when relevant.
- **Link Resolver** -- Register/resolve/fallback system for cross-references. Handles both USC (`/us/usc/`) and CFR (`/us/cfr/`) identifier schemes, producing relative Markdown links within the converted corpus. Unresolved USC references fall back to OLRC website URLs; unresolved CFR references render as plain text.
...
See [USC Package](../packages/usc.md) and [eCFR Package](../packages/ecfr.md) for source-specific details.
...
All internal dependencies use the pnpm `workspace:*` protocol. Turborepo builds packages in topological order: `core` first, then `usc` and `ecfr` in parallel, then `cli`. All packages use lockstep versioning managed by [Changesets](https://github.com/changesets/changesets).
