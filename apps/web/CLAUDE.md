...
**Goal**: Section, chapter, and title viewer pages render with SSR, content provider abstraction, and the shared ContentViewer.
...
1. **Add web app to the monorepo**
   - Create `apps/web/` directory
   - Create `apps/web/package.json` with `"name": "web"`, `"private": true`, `"type": "module"`
   - Verify `pnpm-workspace.yaml` includes `"apps/*"`
   - Initialize Next.js 15 with App Router and TypeScript
   - Do NOT set `output: 'export'` — this site uses SSR
   ...
   - Create `.env.local` with `CONTENT_STORAGE=fs` and `CONTENT_DIR=./content`
   - Run `pnpm install` from monorepo root

2. **Type definitions** — `src/lib/types.ts`
   - All interfaces from the TypeScript Interfaces section above

3. **Content provider** — `src/lib/content/`
   - `types.ts` — `ContentProvider` and `NavProvider` interfaces
   - `fs-provider.ts` — filesystem implementation
   - `index.ts` — provider factory with `getContentProvider()` and `getNavProvider()`

4. **Markdown processing** — `src/lib/markdown.ts`
   - `parseFrontmatter()` — uses gray-matter
   - `renderMarkdownToHtml()` — unified pipeline, module-level singleton processor

5. **Shiki highlighting** — `src/lib/shiki.ts`
   - `highlightMarkdown()` — dual-theme output, module-level singleton highlighter

6. **Section viewer page** — `src/app/usc/[title]/[chapter]/[section]/page.tsx`
   - Dynamic route (no `generateStaticParams`)
   - Reads from content provider at `section/usc/{title}/{chapter}/{section}.md`
   - `generateMetadata()` for SEO

7. **Chapter viewer page** — `src/app/usc/[title]/[chapter]/page.tsx`
   - Reads from `chapter/usc/{title}/{chapter}.md`

8. **Title viewer page** — `src/app/usc/[title]/page.tsx`
   - Reads from `title/usc/{title}.md`
   - Test with Title 1 to verify Shiki handles title-level files

9. **ContentViewer** — `src/components/content/content-viewer.tsx`
   - Single client component for all three granularities
   - Source/rendered toggle, copy, download

10. **FrontmatterPanel** — adapts to granularity via field presence

11. **Copy button** + **Download button**

12. **Root layout** — `src/app/layout.tsx`

13. **Verification**
    - `cd apps/web && pnpm dev`
    - `/usc/title-01/` — title viewer works (title-01.md from title output)
    - `/usc/title-01/chapter-01/` — chapter viewer works
    - `/usc/title-01/chapter-01/section-1/` — section viewer works
    - All three: toggle, copy, download, frontmatter panel all work
    - `/usc/title-99/` — returns 404
    - Check browser network tab: no client-side content fetches (content embedded in HTML)
    - App starts in < 5 seconds (NOT a 30-minute build)

---

### Phase 2 — Navigation & Layout

**Goal**: Full sidebar with lazy loading, virtualization, responsive layout.

**Content setup**: Titles 1, 5, 26 at all three granularities + `npx tsx scripts/generate-nav.ts`.

**Tasks**:

1. **`scripts/generate-nav.ts`** — reads from `content/section/usc/` `_meta.json` files, writes `public/nav/titles.json` and `public/nav/title-{NN}.json`

2. **USC layout** — sidebar + content pane

3. **Sidebar** — title accordion → chapter accordion → section list (virtualized > 100)
   - Title name → navigates to title viewer
   - Chapter name → navigates to chapter viewer
   - Section → navigates to section viewer

4. **Breadcrumbs** — U.S. Code > Title > Chapter > Section

5. **USC index page** — title grid from `NavProvider.getTitles()`

6. **Landing page**

7. **Mobile responsive** — hamburger sidebar, sheet overlay

8. **`scripts/generate-content.sh`** — runs all three convert commands

9. **Verification** — Title 26 sidebar virtualization, navigation between levels

---

### Phase 3 — Search & Polish

**Goal**: Search, dark mode, production polish.

**Content setup**: Titles 1, 5, 18, 26, 42.

**Tasks**:

1. **Pagefind** — index `.md` source files, Cmd+K dialog
2. **Dark mode** — theme toggle, Shiki dual themes
3. **404 page**
4. **SEO** — favicon, Open Graph, robots.txt, sitemap generation from `_meta.json`
5. **Polish** — loading states, smooth scroll, typography

---

### Phase 4 — Full Content & Launch

**Goal**: Production deployment with all 60k+ sections.

**Tasks**:

1. **Full content generation** at all three granularities
2. **Generate navigation JSON + search index + sitemap**
3. **Deploy** — `vercel deploy --prod` (app deploy takes seconds)
4. **Verify** — spot-check Title 1 § 1, Title 26 § 401, Title 42 § 1983
5. **CDN cache warm-up** — optionally crawl critical pages to pre-warm cache
6. **Custom domain**
7. **Performance audit** — Lighthouse, TTFB < 50ms cached / < 200ms uncached
8. **Submit sitemap** to Google Search Console
9. **Document update workflow** — content generation, upload, cache purge
