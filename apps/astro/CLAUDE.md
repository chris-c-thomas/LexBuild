...
- **No sidebar**: `SourceConfig.hasSidebar` is `false` for FR. BaseLayout renders a centered max-width `<main>` instead of the sidebar + content split.
...
- **`hasSidebar` on SourceConfig controls layout**: When `false`, BaseLayout renders a centered max-width `<main>` without sidebar or mobile nav tree. Used for FR (flat, date-based corpus). Check `SOURCES[source]?.hasSidebar !== false` in BaseLayout.
