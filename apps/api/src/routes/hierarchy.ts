    ...
    return c.json(
      {
        data: rows.map((r) => ({
          ...
          url: `/api/v1/usc/titles/${r.title_number}`,
        })),
    ...
    return c.json(
      {
        data: rows.map((r) => ({
          ...
          chapter_count: r.chapter_count,
          positive_law: r.positive_law === 1,
          url: `/api/v1/cfr/titles/${r.title_number}`,
        })),
        meta: { api_version: "v1", timestamp: new Date().toISOString() },
      },
      200,
    ...
    return c.json(
      {
        data: {
          ...
          months: months.map((m) => ({
            ...
            url: `/api/v1/fr/years/${year}/${String(m.month).padStart(2, "0")}`,
          })),
        },
        meta: { api_version: "v1", timestamp: new Date().toISOString() },
      },
