...
- **Download mechanism** uses `<template>` element + Blob URL (not `data:` URI). The `<template>` holds the Markdown content without rendering to the DOM; a top-level `<script>` creates a Blob URL on click. This avoids the ~2MB browser data URI size limit for large titles.
