...
- **Search proxy required in production**: `MEILI_URL=/api` in `.env.production` — the browser cannot reach `127.0.0.1:7700` (that's the user's localhost). Caddy proxies `/api/search` to Meilisearch and injects the auth header.
