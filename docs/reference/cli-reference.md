...
Complete reference for the `lexbuild` command-line tool. Install via `npm install -g @lexbuild/cli` or use `npx @lexbuild/cli`.
...
Specify exactly one of three mutually exclusive input modes:
...
Same as `convert-usc` -- specify exactly one of three mutually exclusive modes:
