...
Definitions for legal, technical, and LexBuild-specific terms used throughout the documentation.
...
**Frontmatter** -- A YAML metadata block at the beginning of each Markdown file, delimited by `---` markers. Contains structured fields (identifier, title, source, legal status, section number, etc.) that enable file-level RAI/RAG ingestion without parsing the Markdown body.
